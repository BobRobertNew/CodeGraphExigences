...
class TestTextUtils(unittest.TestCase):
    ...
    @patch('graph_tool.utils.text_utils.process.extractOne')
    def test_find_best_match_incomplete_tuple(self, mock_extractOne):
        mock_extractOne.return_value = ("apple",)
        with self.assertRaises(IndexError):
            find_best_match("apple", ["apple"])

    @patch('graph_tool.utils.text_utils.process.extractOne')
    def test_find_best_match_with_score_incomplete_tuple(self, mock_extractOne):
        mock_extractOne.return_value = ("apple",)
        with self.assertRaises(IndexError):
            find_best_match_with_score("apple", ["apple"])

    def test_find_best_match_infinity_threshold(self):
        choices = ["apple", "banana"]
        self.assertIsNone(find_best_match("apple", choices, threshold=float('inf')))
        self.assertEqual(find_best_match("apple", choices, threshold=float('-inf')), "apple")

    def test_find_best_match_with_score_infinity_threshold(self):
        choices = ["apple", "banana"]
        match1, score1 = find_best_match_with_score("apple", choices, threshold=float('inf'))
        self.assertIsNone(match1)
        self.assertIsNone(score1)
        match2, score2 = find_best_match_with_score("apple", choices, threshold=float('-inf'))
