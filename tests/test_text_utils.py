...
class TestTextUtils(unittest.TestCase):
    ...
    def test_find_best_match_fuzzy_matching_edge_cases(self):
        self.assertIsNone(find_best_match("query", ["", " "]))
        self.assertEqual(find_best_match("test_#_!", ["test_#_!", "other"], threshold=100), "test_#_!")

    def test_find_best_match_with_score_fuzzy_matching_edge_cases(self):
        match, score = find_best_match_with_score("query", ["", " "])
        self.assertIsNone(match)
        self.assertIsNone(score)
        match, score = find_best_match_with_score("test_#_!", ["test_#_!", "other"], threshold=100)
        self.assertEqual(match, "test_#_!")
        self.assertEqual(score, 100)
