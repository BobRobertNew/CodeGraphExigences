...
class TestTextUtils(unittest.TestCase):
    ...
    def test_find_best_match_query_with_regex_chars(self):
        self.assertEqual(find_best_match("a.*b", ["a.*b", "acb"]), "a.*b")

    def test_find_best_match_query_longer_than_choices(self):
        self.assertEqual(find_best_match("this is a very long string", ["short", "tiny"]), None)

    @patch('graph_tool.utils.text_utils.process.extractOne')
    def test_find_best_match_float_score_exact_threshold(self, mock_extractOne):
        mock_extractOne.return_value = ("apple", 70.0, 0)
        self.assertEqual(find_best_match("appl", ["apple"], threshold=70), "apple")
