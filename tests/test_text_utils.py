...
class TestTextUtils(unittest.TestCase):
    ...
    # ---------------------------------------------------------
    # NEW EXPLICIT EDGE CASE TESTS FOR find_best_match
    # ---------------------------------------------------------

    def test_find_best_match_edge_case_empty_query(self):
        self.assertIsNone(find_best_match("", ["apple", "banana"]))

    def test_find_best_match_edge_case_empty_choices(self):
        self.assertIsNone(find_best_match("apple", []))

    def test_find_best_match_edge_case_none_query(self):
        self.assertIsNone(find_best_match(None, ["apple", "banana"]))

    def test_find_best_match_edge_case_none_choices(self):
        self.assertIsNone(find_best_match("apple", None))

    @patch('graph_tool.utils.text_utils.process.extractOne')
    def test_find_best_match_edge_case_mock_no_match(self, mock_extractOne):
        mock_extractOne.return_value = None
        self.assertIsNone(find_best_match("apple", ["banana"]))

    @patch('graph_tool.utils.text_utils.process.extractOne')
    def test_find_best_match_edge_case_threshold_met_exactly(self, mock_extractOne):
        mock_extractOne.return_value = ("apple", 70, 0)
        self.assertEqual(find_best_match("apple", ["apple", "banana"], threshold=70), "apple")

    @patch('graph_tool.utils.text_utils.process.extractOne')
    def test_find_best_match_edge_case_threshold_missed_slightly(self, mock_extractOne):
        mock_extractOne.return_value = ("apple", 69.9, 0)
        self.assertIsNone(find_best_match("apple", ["apple", "banana"], threshold=70))
