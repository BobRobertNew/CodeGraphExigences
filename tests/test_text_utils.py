...
class TestTextUtils(unittest.TestCase):
    ...
    def test_find_best_match_control_characters(self):
        choices = ["a\x00b", "a\x1fb"]
        self.assertEqual(find_best_match("a\x00b", choices, threshold=100), "a\x00b")

    def test_find_best_match_very_similar_strings_tie(self):
        choices = ["applee", "appel", "apple"]
        self.assertEqual(find_best_match("apple", choices), "apple")

    def test_find_best_match_extremely_long_list(self):
        choices = ["wrong"] * 10000 + ["correct"]
        self.assertEqual(find_best_match("correct", choices), "correct")

    def test_empty_string_match(self):
        self.assertEqual(find_best_match("a", [""], threshold=0), "")

    def test_no_match(self):
        self.assertIsNone(find_best_match("abc", ["xyz"], threshold=50))

    def test_single_choice(self):
        self.assertEqual(find_best_match("abc", ["abc"]), "abc")
