...
class TestTextUtils(unittest.TestCase):
    ...
    def test_find_best_match_regex_special_characters(self):
        # Ensure regex-like characters in the query are treated literally and matched correctly
        choices = ["a.*b", "^regex$", "[a-z]+"]
        self.assertEqual(find_best_match("^regex$", choices, threshold=100), "^regex$")

    def test_find_best_match_extremely_long_strings(self):
        # Test boundary with very long strings
        long_str = "a" * 10000
        choices = [long_str, "b" * 10000]
        self.assertEqual(find_best_match(long_str, choices, threshold=90), long_str)
