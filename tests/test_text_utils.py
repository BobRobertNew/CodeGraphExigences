...
class TestTextUtils(unittest.TestCase):
    ...
    def test_find_best_match_whitespace_variations(self):
        choices = ["a b c", "a  b  c", " a b c "]
        self.assertEqual(find_best_match("a  b c", choices, threshold=90), "a b c")

    def test_find_best_match_highest_score_chosen(self):
        choices = ["pineapple", "apple pie", "apple juice"]
        match = find_best_match("apple", choices, threshold=50)
        self.assertEqual(match, "pineapple")

    def test_find_best_match_very_similar_strings(self):
        choices = ["test1", "test2", "test3"]
        self.assertEqual(find_best_match("test1", choices, threshold=99), "test1")
        self.assertIsNone(find_best_match("test1", ["test2"], threshold=99))
